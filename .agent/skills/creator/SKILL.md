...
- **Referenced scripts**:
  ...
  - Make sure scripts respect `.env` file follow this order: `process.env` > `.agent/skills/${SKILL}/.env` (global) > `.agent/skills/.env` (global) > `$HOME/.claude/.env` (global) > `./.agent/skills/${SKILL}/.env` (cwd) > `./.agent/skills/.env` (cwd) > `./.claude/.env` (cwd)
...
**File size:** Under 150 lines, if you need more, split it to multiple files (<150 lines each) in `resources` folder.
...
- **Best practice**: If files are large (>150 lines), split them into multiple files (<150 lines each) in `resources` folder, include grep search patterns in `SKILL.md`
- **Avoid duplication**: Information should live in either `SKILL.md` or `resources` files, not both. Prefer `resources` files for detailed information unless it's truly core to the skill—this keeps `SKILL.md` lean while making information discoverable without hogging the context window. Keep only essential procedural instructions and workflow guidance in `SKILL.md`; move detailed reference material, schemas, and examples to `resources` files.
...
Also, delete any example files and directories not needed for the skill. The initialization script creates example files in `scripts/`, `resources/`, and `assets/` to demonstrate structure, but most skills won't need all of them.
...
```bash
scripts/package_skill.py <path/to/folder>
```

Optional output directory specification:

```bash
scripts/package_skill.py <path/to/folder> ./dist
```
...
- **Quick checklist**: `resources/validation-checklist.md`
- **Metadata quality**: `resources/metadata-quality-criteria.md`
- **Token efficiency**: `resources/token-efficiency-criteria.md`
- **Script quality**: `resources/script-quality-criteria.md`
- **Structure & organization**: `resources/structure-organization-criteria.md`
